...
- [x] Step 1: Write failing test `tests/test_packaging.py`: plugin.json parses, has name "flight-recorder", version, license MIT; marketplace.json parses, has name "agent-flight-recorder", owner.name, plugins[0].source == "./" and plugins[0].name == "flight-recorder".
- [x] Step 2: Run `uv run pytest tests/test_packaging.py -v`, expect FAIL (files missing).
- [x] Step 3: Create all files listed above.
- [x] Step 4: Run test, expect PASS.
- [x] Step 5: Commit "feat: repo scaffolding, plugin and marketplace manifests".
...
- [x] Step 1: Write failing tests: entries chained from GENESIS; hash covers seq/ts/run_id (mutating any of them breaks verification); resume after reopen; head anchor file written and matches; dev-key warning emitted when env missing.
- [x] Step 2: Run `uv run pytest tests/test_audit_chain.py -v`, expect FAIL.
- [x] Step 3: Implement chained_log.py (stdlib only: json, hmac, hashlib, os, time, pathlib).
- [x] Step 4: Run tests, expect PASS.
- [x] Step 5: Commit "feat: hash-chained HMAC audit logger with truncation anchor".
...
- [x] Step 3: Implement verify_chain.py (imports compute_hash/canonical/GENESIS from chained_log.py via sibling import with fallback path insert, so the folder works when copied standalone).
- [x] Step 4: Run tests, expect PASS.
- [x] Step 5: Commit "feat: chain verifier with anchor enforcement and tamper demo".
...
- [x] Step 1: Write files. SKILL.md body matches the approved draft with the amendments.
- [x] Step 2: `uv run ruff check skills/`, expect clean.
- [x] Step 3: Commit "feat: audit-log skill doc and integration templates".
...
- [x] Step 1: Write failing tests: fixture converts; MCP span name split works with mcp.method.name recognized; turn synthesis ordering; classification derivation table; resource attr merge; skipped-span counting; --map override.
- [x] Step 2: Run, expect FAIL.
- [x] Step 3: Implement.
- [x] Step 4: Run, expect PASS.
- [x] Step 5: Commit "feat: OTLP to triage converter with pack-native default mapping".
...
- [x] Step 1: Write files.
- [x] Step 2: Commit "feat: trace-triage skill doc and mapping reference".
...
- [x] Step 1: Write failing tests: configure_otel with file_export_path writes valid OTLP JSON lines the Task 5 converter can parse; traced_node sets span name, attrs, ERROR status and failure.classification on a classified exception.
- [x] Step 2: Run, expect FAIL (deps: add otel packages to dev extras first).
- [x] Step 3: Implement both templates.
- [x] Step 4: Run, expect PASS.
- [x] Step 5: Commit "feat: otel setup template with OTLP JSON file exporter and langgraph node tracing".
...
- [x] Step 1: Write the YAML parse test (pyyaml in dev deps), expect FAIL.
- [x] Step 2: Write all assets, references, SKILL.md. semconv.md carries the exact vocabulary from the audit findings (mcp.method.name, span name format, recommended attrs, four duration histograms). conformance.md documents the Weaver v0.23.0 live-check loop.
- [x] Step 3: Run tests, expect PASS. If docker available locally run `docker compose -f skills/otel-agent-tracing/assets/docker-compose.yml config -q` once as a sanity check (not in CI).
- [x] Step 4: Commit "feat: local observability stack assets and tracing skill doc".
...
- [x] Step 1: `uv pip install langgraph` in the dev venv, record version, write requirements.txt range around it.
- [x] Step 2: Write failing tests: same seed twice gives identical run summaries; --audit produces a verifiable chain (import verify from Task 3); failure injection yields at least two distinct classifications across 10 runs.
- [x] Step 3: Run, expect FAIL.
- [x] Step 4: Implement app.py and stub_llm.py. Audit integration follows skills/audit-log/templates/langgraph_integration.py; tracing follows templates/langgraph_tracing.py (import via path, no packaging tricks: the example vendors nothing, it inserts the skills paths).
- [x] Step 5: Run, expect PASS.
- [x] Step 6: Commit "feat: deterministic two-agent example app".
...
- [x] Step 1: Write failing test: copy toy app to tmp, materialize evals/ from the templates (string substitution of the entrypoint import), run `pytest evals/ --update-golden` then `pytest evals/` twice via subprocess, both exit 0; goldens exist; corrupt one golden byte, rerun, exit != 0.
- [x] Step 2: Run, expect FAIL.
- [x] Step 3: Write templates. test_golden.py implements --update-golden via a conftest pytest_addoption. harness.py template contains a working default for the toy app shape plus clear EDIT ME markers.
- [x] Step 4: Run, expect PASS.
- [x] Step 5: Commit "feat: eval scaffold templates proven against the example app".
...
- [x] Step 1: Write both tests, run, fix any violations they surface.
- [x] Step 2: Commit "test: skill format and hygiene gates".
...
- [x] Step 2: Validate with `uv run --with pyyaml python -c "import yaml,sys; yaml.safe_load(open('.github/workflows/ci.yml'))"` and run the exact pytest command locally, expect >= 90 percent coverage and all green.
- [x] Step 3: Commit "ci: lint, coverage-gated tests, best-effort plugin validation".
...
- [x] Step 1: Write all four files. README quickstart commands must be copy-paste runnable against the repo as committed.
- [x] Step 2: Manually execute the quickstart commands from a clean tmp clone, confirm under 60 seconds and working.
- [x] Step 3: Run the full suite one final time plus ruff plus hygiene, all green.
- [x] Step 4: Commit "docs: README with vhs demo script, changelog, launch summary".

### Task 15: final verification sweep

- [x] Step 1: `uv run pytest --cov=skills --cov=examples --cov-report=term-missing --cov-fail-under=90` green.
- [x] Step 2: `claude plugin validate . --marketplace` if available locally; otherwise note for CI.
- [x] Step 3: Fresh-clone manual install path check: copy skills/* into a tmp dir shaped like ~/.claude/skills and confirm self-containment (verify_chain.py runs standalone).
- [x] Step 4: Update plan checkboxes, commit "chore: final verification pass".
